...
<div class="d-flex align-items-center gap-2 mb-4">
    ...
    <h1 class="mb-0 h4">Editar totem <span class="text-muted fw-normal">#<?= esc($totem['id']) ?></span></h1>
</div>
...
<div class="card shadow-sm" style="border-top:4px solid #fd7e14;border-radius:.5rem">
    <div class="card-body p-4">
        <form action="<?= site_url('totens/atualizar/' . $totem['id']) ?>" method="post">
            <?= csrf_field() ?>

            <div class="mb-4">
                <label for="descricao" class="form-label small fw-semibold">Descrição</label>
                <input type="text" name="descricao" id="descricao" class="form-control"
                    value="<?= esc(old('descricao', $totem['descricao'])) ?>"
